...
const writerCsvSql = csvWriter();
const writerCsvNoSql = csvWriter();
...
const dataGenerator = () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    output.listing_name.push(faker.company.companyName());
  }
  return output;
}
...
const saveUserToFile = (numbOfUsers) => {
  ...
  for (let i = 1; i <= numbOfUsers; i++) {
    ...
    let id = i;
    writerUser.write({id, username, profile_pic_url})
  }
  writerUser.end();
};
...
const saveDataToFile = (numbOfReviews) => {
  fs.readFile(path.join(__dirname, './dataSets/usersTable.csv'), 'utf8', (err, result) => {
    ...
    writerCsvSql.pipe(fs.createWriteStream(path.join(__dirname,'./dataSets/fakeDataSQL4.csv')));
...
setTimeout(() => saveDataToFile(10000000), 000);